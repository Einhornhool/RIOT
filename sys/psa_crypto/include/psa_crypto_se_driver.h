 ...
 * @{
 *
 * @brief       PSA Crypto Secure Element APIs
 *
 ...
 *          cipher operation
 *
 *          Rationale: The @c psa_se_cipher_* operation in the PSA Cryptographic API has
...
/**
 * @defgroup    Authenticated Encryption with Additional Data (AEAD) operations with secure
 ...
 *
 *          To create a key in a specific slot in a secure element, the core first calls this
 ...
 *          See the documentation of @ref sa_drv_se_allocate_key_t for more details.
 *
 ...
 *
 *          This function can support any output from @ref psa_export_key(). Refer to the
 ...
 *
 *          The output of this function can be passed to @ref psa_import_key() to
 ...
 *
 * @param   drv_context     The driver context structure.
 * @param   key             Slot whose content is to be exported. This must be an occupied key slot.
 ...
 *
 *          If the key type @c type recorded in @c attributes is asymmetric (@ref
 ...
 *          @ref psa_drv_key_management_t::p_export_public function. Some implementations do not
 ...
 *                          @ref psa_drv_key_management_t::p_export_public function to export the
 ...
 *              @ref psa_drv_se_key_derivation_derive and @ref psa_drv_se_key_derivation_export.
 *              @ref psa_drv_se_key_derivation_derive is used when the key material should be
 *              placed in a slot on the hardware and not exposed to the caller.
 *              @ref psa_drv_se_key_derivation_export is used when the key material should be
 *              returned to the PSA Cryptographic API implementation.
 *
 *              Different key derivation algorithms require a different number of inputs.
 *              Instead of having an API that takes as input variable length arrays, which
 *              can be problemmatic to manage on embedded platforms, the inputs are passed
 *              to the driver via a function, @ref psa_drv_se_key_derivation_collateral, that
 ...
 *
 *              @code
 *              @ref psa_drv_se_key_derivation_setup(kdf_algorithm, source_key,
 *              dest_key_size_bytes);
 *              @ref psa_drv_se_key_derivation_collateral(kdf_algorithm_collateral_id_0,
 *                                                   p_collateral_0,
 *                                                   collateral_0_size);
 *              @ref psa_drv_se_key_derivation_collateral(kdf_algorithm_collateral_id_1,
 *                                                   p_collateral_1,
 *                                                   collateral_1_size);
 *              @ref psa_drv_se_key_derivation_collateral(kdf_algorithm_collateral_id_2,
 *                                                   p_collateral_2,
 *                                                   collateral_2_size);
 *              @ref psa_drv_se_key_derivation_derive();
 *              @endcode
 *
 *              key agreement example:
 *              @code
 *              @ref psa_drv_se_key_derivation_setup(alg, source_key. dest_key_size_bytes);
 *              @ref psa_drv_se_key_derivation_collateral(DHE_PUBKEY, p_pubkey, pubkey_size);
 *              @ref psa_drv_se_key_derivation_export(p_session_key,
 *                                               session_key_size,
 *                                               &session_key_length);
 *              @endcode
 */
 ...
 *
 *          Future versions of this specification may add extra substructures at the end of this
 *          structure.
 */
typedef struct {
    /**
