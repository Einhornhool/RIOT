 ...
 *
 *          Algorithms defined by this standard will never have the @ref PSA_ALG_VENDOR_FLAG
 ...
 *
 *          A key with a minimum-tag-length AEAD wildcard algorithm as permitted algorithm policy
 ...
 *
 *          An AEAD algorithm with a shortened tag is similar to the corresponding AEAD algorithm,
 ...
 *
 *          This value can be used to form the permitted algorithm attribute of a key policy for a
 ...
 *              // or VERIFY_MESSAGE
 *              @ref psa_set_key_algorithm(@p &attributes, PSA_xxx_SIGNATURE(@ref PSA_ALG_ANY_HASH));
 *              @endcode
 ...
 *
 *          A key with a minimum-MAC-length MAC wildcard algorithm as permitted algorithm policy
 ...
 *
 *          This is the HMAC-based Extract-and-Expand Key Derivation Function (HKDF) specified by
 ...
 *          algorithm.
 *
 *          For example, @ref PSA_ALG_HMAC(@ref PSA_ALG_SHA_256) is HMAC-SHA-256.
 ...
 *          value.
 *
 *          This macro identifies algorithms that can be used with @ref psa_sign_hash() that use
 ...
 *
 *          HKDF is a family of key derivation algorithms that are based on a hash function and the
 *          HMAC construction.
 *
 * @param   alg An algorithm identifier: a value of type @ref psa_algorithm_t.
 *
 ...
 *
 *          A key-stretching or password-hashing algorithm is a key derivation algorithm that is
 ...
 *
 *          A raw key agreement algorithm is one that does not specify a key derivation function.
 ...
 *
 *          An RSA PSS signature algorithm that requires the standard salt length is constructed
 ...
 *
 *          This includes all algorithms such that @ref PSA_ALG_IS_HASH_AND_SIGN() is true,
 ...
 *
 *          Wildcard algorithm values can only be used to set the permitted algorithm field in a
 ...
 *
 *          The CMAC algorithm must be used with a key for a block cipher. For example, when used
 ...
 *          Modes of Operation: the CMAC Mode for Authentication [SP800-38B].
 *
 *          @b Compatible @b key @b types
 *          - @ref PSA_KEY_TYPE_AES
 *          - @ref PSA_KEY_TYPE_ARIA
 ...
 *
 *          The underlying block cipher is determined by the key type. This symmetric cipher mode
 ...
 *
 *          XTS is a cipher mode which is built from a block cipher, designed for use in disk
 ...
 *          Protection of Data on Block-Oriented Storage Devices [IEEE-XTS].
 *
 *          @b Compatible @b key @b types
 *          - @ref PSA_KEY_TYPE_AES
 *          - @ref PSA_KEY_TYPE_ARIA
 ...
 *
 *          CTR is a stream cipher which is built from a block cipher. The underlying block cipher
 ...
 *
 *          The underlying block cipher is determined by the key type. This is the variant of CFB
 ...
 *          Recommendation for Block Cipher Modes of Operation: Methods and Techniques [SP800-38A],
 *          using a segment size s equal to the block size b. The definition in [SP800-38A] is
 ...
 *
 *          The underlying stream cipher is determined by the key type. The ARC4 and ChaCha20
 ...
 *          IETF Protocols [RFC7539] §2.4, which has a 96-bit nonce and a 32-bit counter.
 *          Implementations can optionally also support the original variant, as defined in ChaCha,
 *          a variant of Salsa20 [CHACHA20], which has a 64-bit nonce and a 64-bit counter. Except
 *          where noted, the [RFC7539] variant must be used.
 *
 ...
 *               -  8 bytes: the cipher operation uses the original [CHACHA20] definition of
 ...
 *              by ChaCha20 and Poly1305 for IETF Protocols [RFC7539], which has a 96-bit nonce and
 *              32-bit counter.
 *          -   An implementation can optionally also support the original variant defined by
 *              ChaCha, a variant of Salsa20 [CHACHA20], which has a 64-bit nonce and 64-bit
 *              counter.
 *
 ...
 *
 *          A truncated MAC algorithm is identical to the corresponding MAC algorithm except that
 ...
 *
 *          This family includes groups with the following key sizes (in bits): 2048, 3072, 4096,
 ...
 *          for Transport Layer Security (TLS) [RFC7919] Appendix A.
 */
 ...
 *
 *          Curve25519 is defined in Curve25519: new Diffie-Hellman speed records [Curve25519].
 *          Curve448 is defined in Ed448-Goldilocks, a new elliptic curve [Curve448].
 *
 *          @b Compatible @b algorithms
 *          - @ref PSA_ALG_ECDH (key pair only)
 */
 ...
 *
 *          This must be a direct input, passed to @ref psa_key_derivation_input_integer().
 */
 ...
 *
 *          This is usually a key of type @ref PSA_KEY_TYPE_PASSWORD passed to
 ...
 *
 *          This is typically a key of type @ref PSA_KEY_TYPE_DERIVE passed to
 ...
 *
 *          A volatile key only exists as long as its identifier is not destroyed. The key material
 ...
 *
 *          This storage area is available on all systems that can store persistent keys without
 ...
 *
 *          This storage location is available on systems that have one or more secure elements
 ...
 *
 *          The size of the key is related to the AES algorithm variant. For algorithms except the
 ...
 *          protocols.
 *
 *          The ARC4 cipher supports key sizes between 40 and 2048 bits, that are multiples of 8.
 *          (5 to 256 bytes)
 *
 ...
 *
 *          The size of the key is related to the Camellia algorithm variant. For algorithms except
 ...
 *          Cipher [NTT-CAM] and also described in A Description of the Camellia Encryption
 *          Algorithm [RFC3713].
 *
 *          @b Compatible @b algorithms
 *          - @ref PSA_ALG_CBC_MAC
 ...
 *          Recommendation for the Triple Data Encryption Algorithm (TDEA) Block Cipher [SP800-67].
 *
 *          @b Compatible @b algorithms
 *          - @ref PSA_ALG_CBC_MAC
 *          - @ref PSA_ALG_CMAC
 ...
 *
 *          The size of an elliptic curve key is the bit size associated with the curve, that is,
 ...
 *
 *          The size of an elliptic curve public key is the same as the corresponding private key.
 ...
 *
 * @note    @ref PSA_HASH_LENGTH(@p alg) provides the output size of hash algorithm alg, in bytes.
 *          @ref PSA_HASH_BLOCK_LENGTH(@p alg) provides the block size of hash algorithm alg,
 *          in bytes.
 *
 *          @b Compatible @b algorithms
 *          - @ref PSA_ALG_HMAC
 ...
 *
 *          This key type is suitable for passwords and passphrases which are typically intended to
 ...
 * @brief   Raw data.
 *
 *          A “key” of this type cannot be used for any cryptographic operation. Applications can
 ...
 *
 *          For the XTS block cipher mode (PSA_ALG_XTS), the SM4 key size is 256 bits (two 16-byte
 *          keys).
 *
 *          The SM4 block cipher is defined in GM/T 0002-2012: SM4 block cipher algorithm
 *          [CSTC0002] (English version [CSTC0002/E]).
 *
 *          @b Compatible @b algorithms
 *          - @ref PSA_ALG_CBC_MAC
 *          - @ref PSA_ALG_CMAC
 ...
 *
 *          This flag allows the implementation to make additional copies of the key material that
 ...
 *
 *          This flag allows the use of @ref psa_copy_key() to make a copy of the key with the same
 ...
 *
 *          This flag allows the key to be used for a key derivation operation or for a key
 ...
 *
 *          This flag allows the use of @ref psa_export_key() to export a key from the
 ...
 *
 *          This flag allows the key to be used to sign a message hash as part of an asymmetric
 ...
 *
 *          This flag allows the key to be used for a MAC calculation operation or for an
 ...
 *
 *          This flag allows the key to be used in a key derivation operation, if otherwise
 ...
 *
 *          This flag allows the key to be used to verify a message hash as part of an asymmetric
 ...
 *
 *          This flag allows the key to be used for a MAC verification operation or for an
 ...
 *          algorithms.
 *
 *          If the size of the signature buffer is at least this large, it is guaranteed that
 ...
 *          key derivation.
 *
 *          This implementation-defined value specifies the maximum length for the PSK input used
