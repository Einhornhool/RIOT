 ...
 *
 *          For algorithms that can be applied to multiple key types, this identifier does not
 ...
 *
 *          The group family identifier is required to create a finite-field Diffie-Hellman
 ...
 *
 *          A key identifier can be a permanent name for a persistent key, or a transient reference
 *          to volatile key.
 */
 ...
 *
 *          The lifetime of a key indicates where it is stored and which application and system
 ...
 *
 *          If an implementation of this API can make calls to external cryptoprocessors such as
 ...
 *
 *          What distinguishes different persistence levels is which device management events can
 ...
 *
 *          This is a structured bitfield that identifies the category and type of key. The range
 ...
 *
 *          Before calling any function on a MAC operation object, the application must initialize
 ...
 *
 *          This is either @ref PSA_SUCCESS, which is zero, indicating success; or a small
 ...
 *
 *          This is the object that represents the metadata of a key object. Metadata that can be
 ...
 *
 *          Before calling any function on a cipher operation object, the application must
 *          initialize it by any of the following means:
 *          - Set the object to all-bits-zero, for example:
 *            @code
