 ...
 */
typedef uint32_t psa_aead_operation_t; /* Implement! */

/**
 * @brief Encoding of a cryptographic algorithm.
 *
 * For algorithms that can be applied to multiple key types, this identifier does not encode the
 * key type. For example, for symmetric ciphers based on a block cipher, @ref psa_algorithm_t
 ...
 * @ref PSA_KEY_TYPE_DH_KEY_PAIR() or @ref PSA_KEY_TYPE_DH_PUBLIC_KEY() macros.
 *
 * The specific Diffie-Hellman group within a family is identified by the @c key_bits attribute of
 * the key.
 */
 ...
 * macros.
 *
 * The specific ECC curve within a family is identified by the @c key_bits attribute of the key.
 */
 ...
 *   @endcode
 * - Initialize the object to the initializer @ref PSA_KEY_DERIVATION_OPERATION_INIT, for example:
 *   @code
 ...
 *
 * A key identifier can be a permanent name for a persistent key, or a transient reference to
 * volatile key.
 */
 ...
 *   @ref PSA_KEY_LIFETIME_GET_PERSISTENCE(lifetime) returns the persistence level for a key
 *   lifetime value.
 ...
 *   information.
 *   @ref PSA_KEY_LIFETIME_GET_LOCATION(lifetime) returns the location indicator for a key lifetime
 *   value.
 *
 * Volatile keys are automatically destroyed when the application instance terminates or on a power
 * reset of the device. Persistent keys are preserved until the application explicitly destroys
 * them or until an implementation-specific device management event occurs, for example, a factory
 * reset.
 *
 * Persistent keys have a key identifier of type @ref psa_key_id_t. This identifier remains valid
 ...
 * - Keys with the lifetime @ref PSA_KEY_LIFETIME_VOLATILE are volatile. All implementations should
 *   support this lifetime.
 * - Keys with the lifetime @ref PSA_KEY_LIFETIME_PERSISTENT are persistent. All implementations
 *   that have access to persistent storage with appropriate security guarantees should support
 *   this lifetime.
 */
 ...
 * (type @ref psa_key_lifetime_t), and encode the location as the upper 24 bits of a 32-bit value.
 *
 * Values for location indicators defined by this specification are shown below:
 * - @c 0:  Primary local storage.
 *          All implementations should support this value. The primary local storage is typically
 *          the same storage area that contains the key metadata.
 * - @c 1:  Primary secure element.
 *          Implementations should support this value if there is a secure element attached to the
 *          operating environment. As a guideline, secure elements may provide higher resistance
 *          against side channel and physical attacks than the primary local storage, but may have
 *          restrictions on supported key types, sizes, policies and operations and may have
 *          different performance characteristics.
 *          @note As for now, this value is not supported by this implementation.
 *          Use the vendor-defined location values.
 * - @c 2–0x7fffff: Other locations defined by a PSA specification.
 *          The PSA Cryptography API does not currently assign any meaning to these locations,
 *          but future versions of this specification or other PSA specifications may do so.
 ...
 * device management events that can affect keys at different persistence levels.
 * The specific management events which affect persistent keys at different levels is outside the
 * scope of the PSA Cryptography specification.
 *
 * @note
 * Key persistence levels are 8-bit values. Key management interfaces operate on lifetimes
 * (type @ref psa_key_lifetime_t), and encode the persistence value as the lower 8 bits of a 32-bit
 * value.
 *
 ...
 *   Implementations should support this value if they support persistent keys at all. Applications
 *   should use this value if they have no specific needs that are only met by
 *   implementation-specific features.
 * - @c 2–127: Persistent key with a PSA-specified lifetime.
 *   The PSA Cryptography specification does not define the meaning of these values, but other PSA
 *   specifications may do so.
 * - @c 128–254: Persistent key with a vendor-specified lifetime.
 *   No PSA specification will define the meaning of these values, so implementations may choose
 *   the meaning freely. As a guideline, higher persistence levels should cause a key to survive
 *   more management events than lower levels.
 * - @c 255 = @ref PSA_KEY_PERSISTENCE_READ_ONLY: Read-only or write-once key.
 *   A key with this persistence level cannot be destroyed. Implementations that support such keys
 *   may either allow their creation through the PSA Cryptography API, preferably only to
 *   applications with the appropriate privilege, or only expose keys created through
 *   implementation-specific means such as a factory ROM engraving process.
 *   Note that keys that are read-only due to policy restrictions rather than due to physical
 *   limitations should not have this persistence level.
 */
 ...
 * - @c 0x0001–0x7fff:
 *   Specification-defined key types. Key types defined by this standard always have bit 15 clear.
 *   Unallocated key type values in this range are reserved for future use.
 * - @c 0x8000–0xffff:
 *   Implementation-defined key types. Implementations that define additional key types must use an
 *   encoding with bit 15 set.
 */
...
/**
 * @brief Public interfaces use size_t, but internally we use a smaller type.
 */
 ...
 *   @endcode
 * - Assign the result of the function @ref psa_mac_operation_init() to the object, for example:
 *   @code
 *   psa_mac_operation_t operation;
 ...
 *
 * This is either @ref PSA_SUCCESS, which is zero, indicating success; or a small negative value
 * indicating that an error occurred. Errors are encoded as one of the @c PSA_ERROR_xxx values
 * defined here.
 */
 ...
 *
 * @note
 * Implementations are recommended to define the attribute object as a simple data structure,
 * with fields corresponding to the individual key attributes. In such an implementation, each
 * function @c psa_set_key_xxx() sets a field and the corresponding function @c psa_get_key_xxx()
 * retrieves the value of the field.
 * An implementations can report attribute values that are equivalent to the original one, but have
 * a different encoding. For example, an implementation can use a more compact representation for
 * types where many bit-patterns are invalid or not supported, and store all values that it does
 * not support as a special marker value. In such an implementation, after setting an invalid
 * value, the corresponding get function returns an invalid value which might not be the one that
 ...
 * resources, the application must call @ref psa_reset_key_attributes(). As an exception, calling
 * @ref psa_reset_key_attributes() on an attribute object is optional if the object has only been
 ...
 *   @endcode
 * - Assign the result of the function @ref psa_key_attributes_init() to the object, for example:
 *   @code
 *   psa_key_attributes_t attributes;
 ...
 * - algorithm:       @ref PSA_ALG_NONE — which does not allow cryptographic usage, but allows
 *                      exporting.
 *
 * Usage
 * A typical sequence to create a key is as follows:
 * -# Create and initialize an attribute object.
 * -# If the key is persistent, call @ref psa_set_key_id(). Also call @ref psa_set_key_lifetime()
 *      to place the key in a non-default location.
 * -# Set the key policy with @ref psa_set_key_usage_flags() and @ref psa_set_key_algorithm().
 * -# Set the key type with @ref psa_set_key_type(). Skip this step if copying an existing key with
 *      @ref psa_copy_key().
 * -# When generating a random key with @ref psa_generate_key() or deriving a key with
 *      @ref psa_key_derivation_output_key(), set the desired key size with @ref psa_set_key_bits().
 ...
 *      attribute object, creates a key with these attributes, and outputs an identifier for the
 *      newly created key.
 * -# Optionally call @ref psa_reset_key_attributes(), now that the attribute object is no longer
 *      needed. Currently this call is not required as the attributes defined in this specification
 *      do not require additional resources beyond the object itself.
 *
