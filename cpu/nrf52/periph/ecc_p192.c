                                                ...
                                                size_t *priv_key_buffer_length, size_t *pub_key_buffer_length)
{
    ...
    *pub_key_buffer_length = PSA_EXPORT_PUBLIC_KEY_OUTPUT_SIZE(attributes->type, attributes->bits);
    ...
    return periph_common_ecc_generate_key_pair(priv_key_buffer, pub_key_buffer, (uint32_t *) priv_key_buffer_length, (uint32_t *) pub_key_buffer_length, CRYS_ECPKI_DomainID_secp192r1);
}
                                        ...
                                        size_t * signature_length)
{
    ...
    return periph_common_ecc_sign_hash(key_buffer, PSA_BITS_TO_BYTES(attributes->bits), hash, hash_length, signature, signature_length, hash_mode, CRYS_ECPKI_DomainID_secp192r1);
}
                                        ...
                                        const uint8_t *signature, size_t signature_length)
{
    ...
    (void) attributes;
    return periph_common_ecc_verify_hash(key_buffer, key_buffer_size, hash, hash_length, signature, signature_length, hash_mode, CRYS_ECPKI_DomainID_secp192r1);
}
