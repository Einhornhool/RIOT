                                            ...
                                            size_t * output_length)
{
    ...
    status = common_aes_setup((SaSiAesUserContext_t *) &operation.backend_ctx.cipher_ctx.aes_128, SASI_AES_ENCRYPT, SASI_AES_MODE_CBC, padding, output, key_buffer, key_buffer_size);
    if (status != PSA_SUCCESS) {
        return status;
    }

    status = common_aes_encrypt((SaSiAesUserContext_t *) &operation.backend_ctx.cipher_ctx.aes_128, input, input_length, output + operation.default_iv_length, output_size, output_length);
    if (status != PSA_SUCCESS) {
        return status;
    }
    return PSA_SUCCESS;
